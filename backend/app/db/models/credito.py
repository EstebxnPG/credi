...
class Credito(Base, TimestampMixin, SoftDeleteMixin):
    ...
    situacion_credito: Mapped[str] = mapped_column(String(30), nullable=False, default="NORMAL")
    motivo_finalizacion: Mapped[str | None] = mapped_column(String(50))
    fecha_reactivacion: Mapped[date | None] = mapped_column(Date)
    observacion_situacion: Mapped[str | None] = mapped_column(Text)
    ...
    @property
    def pagaduria_nombre(self) -> str | None:
        return self.pagaduria.nombre if self.pagaduria else None
