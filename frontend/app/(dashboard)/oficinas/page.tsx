    ...
    <section className="space-y-4">
      ...
      {!loading && !error ? (
        <CatalogTable headers={["Nombre", "Direccion", "Estado", "Acciones"]}>
          ...
          {filtered.length === 0 ? <EmptyState text="No hay oficinas para la busqueda actual." /> : null}
        </CatalogTable>
      ) : null}
      {modalMode ? (
...
function Modal({ title, error, saving, submitLabel, onClose, onSubmit, children }: { title: string; error: string | null; saving: boolean; submitLabel: string; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/35 px-4 py-6 backdrop-blur-sm">
      <form onSubmit={onSubmit} className="max-h-[calc(100vh-48px)] w-full max-w-2xl overflow-auto rounded-2xl border border-stone-800/10 bg-white p-5 shadow-2xl shadow-stone-950/20">
